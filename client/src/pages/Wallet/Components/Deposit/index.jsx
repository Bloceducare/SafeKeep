...
function Deposit({showModal= false, operationType ='Deposit'}) {
  ...
  const {data:{id}, crud} = useSelector(vault)
...
const newTok =  {...tok, userTokenAmt:0}
    ...
    const tokenValue = (Number(coin?.balance))/Math.pow(10, coin?.decimals)*price
...
<Balance>
               Balance:  <span>
                 {tkBal(nativeTokenObj)}             
                 </span>
               </Balance>
               <div>
               ~${tkValue(nativeTokenObj, tokenPrice) ?? 0}        
               </div>
               </SpaceBetween>
...
const _ERC20Bal =  <form onSubmit ={handleTransaction}>
...
{selectedAssets.map((item, idx)=>(
  <>
  <div style ={{position:'relative', margin:'0.5rem 1.5rem', marginTop:`${idx===0 ? '2rem':'1.2rem'}`}}>
  <SpaceBetween>
...
const _view = tokenType ==='native'  ? _nativeCoin :tokenType==='erc20Tokens' ? _ERC20Bal:null
    ...
    <h6 style={{marginTop:'1.5rem'}}>Deposit Type </h6>
               <div className="d-flex align-items-center justify-content-between" style ={{maxWidth:'15rem', margin:'1rem 0'}} >
               <div class="custom-control custom-radio custom-control-inline">
                
  <input type="radio" id="customRadioInline1" name="tokenType" value='native' onChange={handleTokenType} class="custom-control-input" />
  <label class="custom-control-label mr-3" for="customRadioInline1" style ={{marginLeft:'0.5rem'}}>
...
<div class="custom-control custom-radio custom-control-inline">
  <input type="radio" id="customRadioInline2" name="tokenType" value ='erc20Tokens' onChange = {handleTokenType} class="custom-control-input" />
  <label class="custom-control-label" for="customRadioInline2" style ={{marginLeft:'0.5rem'}}>Others</label>
</div>
                 </div>      
